...
esp_err_t ts_coordination_fetch_peers(ts_t *ml) {
    ...
    // Allocate only as much as the received HTTP/2 data (h2_buffer_len is a safe
    // upper bound for the DATA-frame payload sum), plus a null-terminator guard.
    // This replaces the previous fixed 64 KB allocation and frees ~40 KB of heap
    // that would otherwise be held during the cJSON parse step.
    ...
    while (pos + 9 <= h2_buffer_len) {
        ...
        if (frame_type == 0x00 && frame_len > 0) {  // DATA frame
            ...
            if (json_total_len + frame_len <= json_buffer_size) {
    ...
    // ========================================================================
    // STEP 2 (SKIPPED): Long-poll disabled for single-core stability.
    // Periodic ts_coordination_poll_updates() in MONITORING state handles
    // keepalive with non-streaming MapRequests every 30s.
    // Endpoints were already advertised in the initial Stream=false MapRequest.
    // ========================================================================
    return ESP_OK;
            ...
            if (enc_buf) {
                ...
                if (ret == ESP_OK) {
                    ...
                    if (sent == 3 + enc_size) {
                        ...
                        // === DISABLED: Core 1 poll task ===
                        // Core 0 handles polling via periodic ts_coordination_poll_updates()
                        // in the MONITORING state. Single-core TCP avoids lwIP issues.
                        /* poll task disabled for single-core operation */
                        (void)0;
                    } else {
...
esp_err_t ts_coordination_heartbeat(ts_t *ml) {
    ...
    // IMPORTANT: No long-poll active — Stream=false heartbeats are safe.
    // Advertise endpoints to keep device visible to other peers.
