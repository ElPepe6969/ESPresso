...
void ts_state_machine(ts_t *ml) {
    ...
    switch (ml->state) {
        ...
        case TS_STATE_MONITORING: {
            ...
            // Skip periodic heartbeats — initial MapRequest already advertises
            // endpoints. Heartbeat function doesn't consume its response,
            // causing socket buffer overflow. Polling handles keepalive every 30s.
